...
namespace Geometry.DCEL
{
    public static class HalfEdgeUtility
    {
        ...
        {
            return polygon.edges.Where(t => { return t.vertex == vertex; }).ToList();
        }
        ...
        public static Dictionary<HalfEdge, VertexHandleType> DetermineVertexType(HalfEdgeFace face,
            ref Dictionary<HalfEdge, VertexHandleType> typeTable)
        {
            if (typeTable == null)
            ...
            for (int i = 0; i < ConnectedEdges.Count; i++)
            {
                ...
                {
                    VertexHandleType type = DetermineType(prevEdge.vertex, ConnectedEdges[i].vertex, nextEdge.vertex,
                        isOuter);
        ...
        public static VertexHandleType DetermineType(HalfEdgeVertex prev, HalfEdgeVertex cur, HalfEdgeVertex next,
            bool outer = true)
        {
            ...
            if (!outer)
            {
                switch (vertexType)
                {
                    // 어떤 케이스는 이게 맞고 아닌 케이스도 있고... 대체 어떻게 해야 하는건데?
                    // 근데 이거는 Edge 연결 문제인거 같은데
