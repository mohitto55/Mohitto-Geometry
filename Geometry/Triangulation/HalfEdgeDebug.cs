...
namespace Geometry.DCEL
{
    ...
    {
        public static IEnumerator CheckEdgeCycle(HalfEdgeData data, HalfEdgeFace face, List<HalfEdgeDebugValue> point,
            float delay = 1)
        {
            Debug.Log("여행 시작");
            ...
            {
                point.Clear();
                point.Add(new HalfEdgeDebugValue(searchEdge.vertex.Coordinate));
        ...
        public static IEnumerator TravelFaceVertex(HalfEdgeData data, HalfEdgeFace face,
            List<HalfEdgeDebugValue> point, float delay = 1)
        {
            ...
            {
                point.Clear();
                point.Add(new HalfEdgeDebugValue(edges[i].vertex.Coordinate, Color.green));
                Debug.Log("<color=green>" + edges[i].vertex.Coordinate + " " + edges[i].next.vertex.Coordinate + " " +
                          edges[i].twin.vertex.Coordinate + "</color>");
        ...
        public static void DebugHalfEdgeData(HalfEdgeData data)
        {
            ...
            for (int i = 0; i < vertices.Count(); i++)
            {
                ...
                foreach (HalfEdge adjacentEdge in halfEdges)
                {
                    ...
                    Debug.Log("prev : " + adjacentEdge.prev.vertex.Coordinate + " / cur : " +
                              adjacentEdge.vertex.Coordinate + " / next : " + adjacentEdge.next.vertex.Coordinate);
                    ...
                    Debug.Log("prev : " + adjacentEdge.twin.prev.vertex.Coordinate + " / cur : " +
                              adjacentEdge.twin.vertex.Coordinate + " / next : " +
                              adjacentEdge.twin.next.vertex.Coordinate);
